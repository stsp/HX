...
#include <arpa/inet.h>
#include <netinet/tcp.h>       /* watt32 include file */
...
LPSOCKET __stdcall _accept( LPSOCKET s, struct sockaddr * paddr, socklen_t * addrlen )
{
    LPSOCKET sock;
    int tmps;
...
int __stdcall __getpeername( LPSOCKET s, struct sockaddr * name, socklen_t * namelen )
{
    DebugOut3("getpeername(%X, %X, %u)\r\n", s, name, namelen);
    return( getpeername( s->hSocket, name, namelen ) );
}

int __stdcall __getsockname( LPSOCKET s, struct sockaddr * name, socklen_t * namelen )
{
#ifdef _DEBUG
...
int __stdcall __getsockopt( LPSOCKET s, int level, int optname, char * optval, int * optlen )
{
    ...
    return( rc );
#else
    return( getsockopt( s->hSocket, level, optname, optval, (socklen_t *)optlen ) );
#endif
}
...
char * __stdcall __inet_ntoa( DWORD in )
{
    ...
    dwAddr.s_addr = ((in & 0xFF) << 24) | ((in & 0xFF00) << 8) | ((in & 0xFF0000) >> 8) | ((in & 0xFF000000) >> 24);
#ifdef _DEBUG
...
int __stdcall _ioctlsocket( LPSOCKET s, long cmd, char * argp )
{
    ...
    return( rc );
#else
    return( ioctlsocket(s->hSocket, cmd, (unsigned *)argp) );
return -1;
#endif
}
...
int __stdcall _recvfrom( LPSOCKET s, void * buf, int len, int flags, struct sockaddr * from, socklen_t * fromlen )
{
...
//    return( gethostname(pname, namelen) );
    strlcpy(pname, "localhost", namelen);
    return 0;
}
